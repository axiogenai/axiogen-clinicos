...
async function sendOTPEmail(toEmail, otp) {
  ...
  // ── Strategy 1: Nodemailer via axiogen01@gmail.com (PRIMARY)
  if (SMTP_PASS) {
    try {
  ...
  // ── Strategy 2: Supabase OTP email (FALLBACK — no SMTP needed)
  try {
    const res = await fetch(`${SUPABASE_URL}/auth/v1/otp`, {
      ...
      body: JSON.stringify({ email: toEmail, create_user: true })
    });
    if (res.ok || res.status === 422) {
      console.log(`⚡ Supabase fallback OTP email sent to ${toEmail}`);
      return true;
    }
  } catch (err) {
    console.log(`ℹ️ Supabase fallback error: ${err.message}`);
  }

  console.warn(`⚠️ All email strategies failed for ${toEmail}`);
  return false;
}
